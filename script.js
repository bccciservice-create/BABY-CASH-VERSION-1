...
const SUPABASE_URL = "https://gbjwighpbakqporvueip.supabase.co";
const SUPABASE_ANON_KEY = "sb_publishable_ENUa23RlRcy7S5mrArt0eQ_4R_TbtaX";
const bccSupabase = window.supabase
? window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY)
: null;

if (bccSupabase) {
console.info("Supabase connecte");
} else {
console.warn("Supabase non charge : l'application continue en mode local.");
}

async function supabaseInsert(table,row){
if(!bccSupabase){
return {ok:false,offline:true,error:"Supabase n'est pas charge."};
}
try{
const {error} = await bccSupabase.from(table).insert(row);
if(error){
console.warn(`Supabase insert ${table} refuse`,error);
return {ok:false,error:error.message || String(error)};
}
return {ok:true};
}catch(error){
console.warn(`Supabase insert ${table} impossible`,error);
return {ok:false,error:error.message || String(error)};
}
}

async function supabaseUpdate(table,values,column,value){
if(!bccSupabase){
return {ok:false,offline:true,error:"Supabase n'est pas charge."};
}
try{
const {error} = await bccSupabase.from(table).update(values).eq(column,value);
if(error){
console.warn(`Supabase update ${table} refuse`,error);
return {ok:false,error:error.message || String(error)};
}
return {ok:true};
}catch(error){
console.warn(`Supabase update ${table} impossible`,error);
return {ok:false,error:error.message || String(error)};
}
}
...
if(!bccSupabase){
return {ok:false,data:[],offline:true,error:"Supabase n'est pas charge."};
}
try{
let query = bccSupabase.from(table).select(columns);
filters.forEach(([column,value])=>{
query = query.eq(column,value);
});
const {data,error} = await query;
if(error){
console.warn(`Supabase select ${table} refuse`,error);
return {ok:false,data:[],error:error.message || String(error)};
}
return {ok:true,data:data || []};
}catch(error){
console.warn(`Supabase select ${table} impossible`,error);
return {ok:false,data:[],error:error.message || String(error)};
}
}

function showSupabaseStatus(result,action){
if(result?.ok || result?.offline){
return;
}
alert(`Action locale enregistree, mais Supabase a refuse ${action} : ${result.error}`);
}
...
function walletToSupabaseRow(user){
return {
user_id:user.id,
main_balance:Number(user.mainBalance || 0),
reward_balance:Number(user.rewardBalance || 0),
monthly_used:Number(user.monthlyUsed || 0),
monthly_limit:getMonthlyLimit(user)
};
}

function transactionToSupabaseRow(transaction,userId){
return {
id:transaction.id,
user_id:userId,
type:transaction.type || transaction.name || "transaction",
amount:Number(transaction.amount || 0),
fees:Number(transaction.fees || 0),
status:transaction.status || "valide",
created_at:new Date().toISOString()
};
}

async function syncUserToSupabase(user){
const userResult = await supabaseInsert("users",userToSupabaseRow(user));
showSupabaseStatus(userResult,"l'inscription utilisateur");
const walletResult = await supabaseInsert("wallets",walletToSupabaseRow(user));
showSupabaseStatus(walletResult,"la creation du wallet");
}

async function syncWalletToSupabase(user){
const result = await supabaseUpdate("wallets",walletToSupabaseRow(user),"user_id",user.id);
showSupabaseStatus(result,"la mise a jour du wallet");
}

async function syncTransactionToSupabase(transaction,userId){
const result = await supabaseInsert("transactions",transactionToSupabaseRow(transaction,userId));
showSupabaseStatus(result,"la transaction");
}
...
console.warn(`Stockage local invalide pour ${key}, reinitialisation.`,error);
localStorage.removeItem(key);
return fallback;
}
}

let appSettings = readJsonStorage(APP_SETTINGS_KEY,{language:"fr",languageName:"Francais"});
let state = loadState();
prepareState();
let session = readJsonStorage("bccSession",null);
...
if(typeof state.settings.personalMaintenanceUserId === "undefined"){ state.settings.personalMaintenanceUserId = ""; changed = true; }
...
void supabaseInsert("notifications",{
id:notification.id,
...
try{
new Notification(title,{body:message,tag:"bcc-notification"});
}catch(error){
console.warn("Notification telephone non envoyee",error);
}
}
...
const dict = i18n[appSettings.language] || i18n.fr;
const frDict = i18n.fr || {};
const reverse = {};
Object.keys(i18n).forEach(code=>{
Object.entries(i18n[code] || {}).forEach(([base,translated])=>{
...
if(value){
if(!element.dataset.i18nBase){
element.dataset.i18nBase = frDict[value] ? value : (reverse[value] || value);
}
if(dict[element.dataset.i18nBase]){
element.innerText = dict[element.dataset.i18nBase];
}
}
}
if(element.placeholder){
if(!element.dataset.i18nPlaceholder){
element.dataset.i18nPlaceholder = frDict[element.placeholder] ? element.placeholder : (reverse[element.placeholder] || element.placeholder);
}
if(dict[element.dataset.i18nPlaceholder]){
element.placeholder = dict[element.dataset.i18nPlaceholder];
}
}
});
const text = document.getElementById("selectedLanguageText");
...
const alreadyAllowed = !!localStorage.getItem(REMEMBER_KEY);
if(!alreadyAllowed && !confirm("Autorisez-vous BABY CASH COIN a memoriser vos infos de connexion sur cet appareil ?")){
...
if(!saveRememberedLogin(email,password,remember)){
return;
}
...
<button class="outline-btn" id="requestForgotOtpBtn" style="width:100%;margin-top:12px">Lancer le code OTP</button>
<p id="forgotOtpStatus" class="mini-status" style="margin-top:10px">Demandez d'abord le code OTP, puis finalisez la requete.</p>
...
setTimeout(()=>{
let expectedOtp = "";
document.getElementById("requestForgotOtpBtn").addEventListener("click",()=>{
...
if(!user){
alert("Renseignez d'abord un email et un numero Mobile Money valides.");
return;
}
expectedOtp = String(Math.floor(100000 + Math.random() * 900000));
user.pendingForgotOtp = expectedOtp;
notifyUser(user,"OTP mot de passe oublie",`Votre code OTP de recuperation est ${expectedOtp}.`,"security");
notifyAdmins("OTP mot de passe oublie",`OTP prepare pour ${user.email}.`,"security");
saveState();
document.getElementById("forgotOtpStatus").innerText = `Code OTP lance. Mode demo/GitHub : ${expectedOtp}`;
});
...
if(!user.pendingForgotOtp || otp !== user.pendingForgotOtp){
alert("Lancez d'abord le code OTP et entrez le code recu.");
return;
}
delete user.pendingForgotOtp;
...
welcome.id = "welcomeShareholderOverlay";
welcome.className = "welcome-shareholder";
welcome.innerHTML = `
...
syncCurrentUser();
if(isUserMaintenanceBlocked()){
renderMaintenanceLock();
return;
}
...
function isUserMaintenanceBlocked(){
if(!currentUser || session?.type !== "user"){
return false;
}
const personalId = String(state.settings.personalMaintenanceUserId || "").trim();
return !!state.settings.globalMaintenance || (!!personalId && personalId === currentUser.id);
}

function renderMaintenanceLock(){
document.querySelectorAll(".page").forEach(page=>page.classList.remove("active-page"));
document.getElementById("homeSection").classList.add("active-page");
document.getElementById("homeSection").innerHTML = `
<div class="personal-maintenance-lock">
<h2>Maintenance BABY CASH COIN</h2>
<p>${state.settings.maintenanceMessage || "Votre compte est temporairement en maintenance."}</p>
<button class="logout-btn" id="maintenanceLogoutBtn" type="button">Deconnecter</button>
</div>
`;
document.getElementById("maintenanceLogoutBtn").addEventListener("click",confirmLogout);
document.querySelectorAll(".bottom-nav button,.actions button,#dailyMissionBtn").forEach(button=>{
...
document.getElementById("levelBadge").className = `badge level-badge level-${level.badge.toLowerCase().replace(/\s+/g,"-")}`;
...
saveState();
await syncWalletToSupabase(currentUser);
await syncTransactionToSupabase(transaction,currentUser.id);
...
<button class="operator-card active" data-operator="Wave" type="button"><span class="operator-icon wave">W</span><strong>Wave</strong></button>
<button class="operator-card" data-operator="Orange Money" type="button"><span class="operator-icon orange">OM</span><strong>Orange</strong></button>
<button class="operator-card" data-operator="MTN Money" type="button"><span class="operator-icon mtn">MTN</span><strong>MTN</strong></button>
<button class="operator-card" data-operator="Moov Money" type="button"><span class="operator-icon moov">M</span><strong>Moov</strong></button>
</div>
...
setTimeout(()=>{
document.querySelectorAll(".operator-card").forEach(button=>{
button.addEventListener("click",()=>{
document.querySelectorAll(".operator-card").forEach(item=>item.classList.remove("active"));
button.classList.add("active");
document.getElementById("withdrawOperator").value = button.dataset.operator;
});
});
...
saveState();
await syncWalletToSupabase(currentUser);
await syncTransactionToSupabase({...withdrawal,type:"withdrawal",fees:bccFees.toFixed(2)},currentUser.id);
...
const targetUserId = String(mission.targetUserId || "").trim();
if(activeMissionView === "personal"){
...
<button class="mission-tab ${activeMissionView === "all" ? "active" : ""}" data-mission-view="all" type="button">Missions pour tous</button>
<button class="mission-tab ${activeMissionView === "personal" ? "active" : ""}" data-mission-view="personal" type="button">Missions personnelles</button>
</div>
${visibleMissions.map(mission=>`
<div class="mission-group">
${mission.image ? `<img class="mission-cover" src="${mission.image}" alt="${mission.title}">` : ""}
<h3>${mission.title}</h3>
${mission.targetUserId ? `<p class="status-pill active">Mission personnalisee</p>` : ""}
...
`).join("") || `<div class="list-row">Aucune mission disponible dans cette page</div>`}
`;

document.querySelectorAll("[data-mission-view]").forEach(button=>{
button.addEventListener("click",()=>{
activeMissionView = button.dataset.missionView;
renderMissions();
});
});
...
askPin("Action sensible : faire une mission",async()=>{
...
await syncWalletToSupabase(currentUser);
await syncTransactionToSupabase(transaction,currentUser.id);
await supabaseInsert("mission_submissions",{
id:uid("SUB"),
user_id:currentUser.id,
mission_id:mission.id,
status:"validated",
gain:Number(mission.gain || 0),
created_at:new Date().toISOString()
});
...
fileToDataUrl(proofFile).then(async proofImage=>{
...
saveState();
showSupabaseStatus(await supabaseInsert("mission_submissions",{
id:submission.id,
user_id:currentUser.id,
mission_id:mission.id,
status:"pending",
...
saveState();
showSupabaseStatus(await supabaseInsert("kyc",{
id:uid("KYC"),
user_id:currentUser.id,
status:"pending",
files:currentUser.kycFiles,
...
document.getElementById("sendSupportRequestBtn").addEventListener("click",async()=>{
syncCurrentUser();
...
state.supportRequests.unshift(supportRequest);
notifyAdmins("Nouvelle requete assistance",`${currentUser.email} : ${message}`,"support");
saveState();
showSupabaseStatus(await supabaseInsert("support_requests",{
id:supportRequest.id,
user_id:supportRequest.userId,
email:supportRequest.email,
message:supportRequest.message,
status:supportRequest.status,
reply:supportRequest.reply,
created_at:new Date().toISOString()
}),"la requete assistance");
...
saveState();
await syncWalletToSupabase(currentUser);
showSupabaseStatus(await supabaseInsert("rewards",{
id:uid("RWD"),
user_id:currentUser.id,
gain:Number(gain || 0),
next_scratch_at:new Date(currentUser.nextScratchAt).toISOString()
}),"la recompense carte cadeau");
...
document.getElementById("transferRewardBtn").addEventListener("click",async()=>{
syncCurrentUser();
...
const personalMaintenanceInput = document.getElementById("personalMaintenanceUserId");
if(personalMaintenanceInput){
personalMaintenanceInput.value = state.settings.personalMaintenanceUserId || "";
}
}
...
<span>Cible : ${mission.targetUserId || "Tous les utilisateurs"}</span>
...
const targetUserId = document.getElementById("adminMissionUserId")?.value.trim() || "";
...
showSupabaseStatus(await supabaseInsert("missions",{
id:mission.id,
title:mission.title,
link:mission.link,
gain:Number(mission.gain || 0),
...
if(document.getElementById("adminMissionUserId")){
document.getElementById("adminMissionUserId").value = "";
}
...
async function updateMissionProof(id,status){
...
saveState();
await supabaseUpdate("mission_submissions",{status},"id",id);
await syncWalletToSupabase(user);
if(status === "validated"){
const createdTx = user.transactions[0];
if(createdTx){
await syncTransactionToSupabase(createdTx,user.id);
}
}
...
state.settings.personalMaintenanceUserId = document.getElementById("personalMaintenanceUserId")?.value.trim() || "";
saveState();
